...
pub const UPDATE_FAMILY: u16 = 0x1;
pub const PRESENCE_FAMILY: u16 = 0x2;
pub const WATERMARK_FAMILY: u16 = 0x3;
pub const OP_FAMILY: u16 = 0x4;
pub const NODE_FAMILY: u16 = 0x5;
pub const GRAFTED_NODE_FAMILY: u16 = 0x6;
pub const CHUNK_FAMILY: u16 = 0x7;
pub const CURRENT_META_FAMILY: u16 = 0x8;

pub const UPDATE_VERSION_LEN: usize = 8;
pub const ORDERED_KEY_ESCAPE_BYTE: u8 = 0x00;
pub const ORDERED_KEY_ZERO_ESCAPE: u8 = 0xFF;
pub const ORDERED_KEY_TERMINATOR_LEN: usize = 2;

pub const UPDATE_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, UPDATE_FAMILY);
pub const PRESENCE_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, PRESENCE_FAMILY);
pub const WATERMARK_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, WATERMARK_FAMILY);
pub const OPERATION_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, OP_FAMILY);
pub const NODE_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, NODE_FAMILY);
pub const CURRENT_META_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, CURRENT_META_FAMILY);
pub const GRAFTED_NODE_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, GRAFTED_NODE_FAMILY);
pub const CHUNK_CODEC: KeyCodec = KeyCodec::new(RESERVED_BITS, CHUNK_FAMILY);

pub const NO_PARTIAL_CHUNK: u64 = 0;

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct UpdateRow<K, V> {
    pub key: K,
    pub value: Option<V>,
}
...
pub const fn bitmap_chunk_bits<const N: usize>() -> u64 {
    (N as u64) * 8
}

pub const fn grafting_height_for<const N: usize>() -> u32 {
    bitmap_chunk_bits::<N>().trailing_zeros()
}

pub fn chunk_index_for_location<const N: usize>(location: Location) -> u64 {
    *location / bitmap_chunk_bits::<N>()
}

pub fn ops_to_grafted_pos(ops_pos: Position, grafting_height: u32) -> Position {
...
pub fn position_height(pos: Position) -> u32 {
...
pub fn decode_digest<D: Digest>(bytes: &[u8], label: String) -> Result<D, QmdbError> {
    if bytes.len() != D::SIZE {
...
pub fn mmr_size_for_watermark(watermark: Location) -> Result<Position, QmdbError> {
...
pub fn ensure_encoded_value_size(len: usize) -> Result<(), QmdbError> {
...
pub fn ordered_key_encoded_len(raw_key: &[u8]) -> usize {
    raw_key.len()
        + raw_key
            .iter()
...
pub fn encode_ordered_key_bytes(raw_key: &[u8]) -> Vec<u8> {
...
pub fn validate_ordered_key_bytes(bytes: &[u8], label: &str) -> Result<(), QmdbError> {
...
pub fn encode_ordered_update_payload(
    codec: KeyCodec,
    raw_key: &[u8],
...
pub fn encode_update_key(raw_key: &[u8], location: Location) -> Result<Key, QmdbError> {
...
pub fn decode_update_location(key: &Key) -> Result<Location, QmdbError> {
    let codec = UPDATE_CODEC;
...
pub fn encode_presence_key(latest_location: Location) -> Key {
...
pub fn encode_watermark_key(location: Location) -> Key {
...
pub fn decode_watermark_location(key: &Key) -> Result<Location, QmdbError> {
    let codec = WATERMARK_CODEC;
    if !codec.matches(key) {
        return Err(QmdbError::CorruptData(
...
pub fn encode_operation_key(location: Location) -> Key {
...
pub fn encode_node_key(position: Position) -> Key {
...
pub fn encode_current_meta_key(location: Location) -> Key {
...
pub fn encode_grafted_node_key(position: Position, watermark: Location) -> Key {
...
pub fn encode_chunk_key(chunk_index: u64, watermark: Location) -> Key {
...
pub fn decode_operation_location_key(key: &Key) -> Result<Location, QmdbError> {
    let codec = OPERATION_CODEC;
    if !codec.matches(key) {
        return Err(QmdbError::CorruptData(
...
pub fn decode_presence_location(key: &Key) -> Result<Location, QmdbError> {
    let codec = PRESENCE_CODEC;
    if !codec.matches(key) {
        return Err(QmdbError::CorruptData(
