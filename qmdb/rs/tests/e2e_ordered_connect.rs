...
async fn ordered_connect_get_returns_current_key_value_proof() {
    ...
    let proof = client
        .get(
            ProtoGetRequest {
                key: b"alpha".to_vec().encode().to_vec(),
...
async fn ordered_connect_get_many_returns_current_key_lookup_proofs() {
    ...
    let proof = client
        .get_many(
            ProtoGetManyRequest {
                keys: vec![
                    b"alpha".to_vec().encode().to_vec(),
                    b"beta".to_vec().encode().to_vec(),
                ],
                tip: local.latest_location.as_u64(),
                ..Default::default()
            },
...
async fn ordered_connect_get_many_returns_miss_proofs_and_rejects_duplicates() {
    ...
    let proof = client
        .get_many(
            ProtoGetManyRequest {
                keys: vec![
                    b"alpha".to_vec().encode().to_vec(),
                    b"aardvark".to_vec().encode().to_vec(),
                ],
                tip: local.latest_location.as_u64(),
                ..Default::default()
            },
    ...
    let aardvark = b"aardvark".to_vec().encode().to_vec();
    assert!(matches!(
        &proof[1],
...
async fn ordered_connect_get_range_verifies_complete_empty_and_partial_pages() {
    ...
    let complete = client
        .get_range(
            ProtoGetRangeRequest {
                start_key: b"a".to_vec().encode().to_vec(),
                end_key: Some(b"c".to_vec().encode().to_vec()),
                limit: 10,
                tip: local.latest_location.as_u64(),
                ..Default::default()
            },
    ...
    let partial = client
        .get_range(
            ProtoGetRangeRequest {
                start_key: b"a".to_vec().encode().to_vec(),
                end_key: Some(b"z".to_vec().encode().to_vec()),
                limit: 1,
                tip: local.latest_location.as_u64(),
                ..Default::default()
            },
    ...
    assert_eq!(partial.next_start_key, b"beta".to_vec().encode().to_vec());

    let empty = client
        .get_range(
            ProtoGetRangeRequest {
                start_key: b"aardvark".to_vec().encode().to_vec(),
                end_key: Some(b"alpha".to_vec().encode().to_vec()),
                limit: 10,
                tip: local.latest_location.as_u64(),
                ..Default::default()
            },
...
async fn ordered_connect_client_rejects_get_range_boundary_omission() {
    ...
    let raw_get_many_response = rpc
        .get_many(ProtoGetManyRequest {
            keys: vec![b"alpha".to_vec().encode().to_vec()],
            tip: local.latest_location.as_u64(),
            ..Default::default()
        })
        .await
...
async fn ordered_connect_client_rejects_get_many_proof_for_different_key() {
    ...
    let mut raw_get_many_response = rpc
        .get_many(ProtoGetManyRequest {
            keys: vec![b"beta".to_vec().encode().to_vec()],
            tip: local.latest_location.as_u64(),
            ..Default::default()
        })
        .await
